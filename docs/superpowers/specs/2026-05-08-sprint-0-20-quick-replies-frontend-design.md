...
real de listagem + criação + edição + soft-delete + reativação,
replicando o padrão consolidado em `/configuracoes/tags` (Sprint 0.19) e
estendendo-o para suportar:
...
Hard delete (`DELETE ?hard=true`) e endpoint dedicado de upload de mídia
(`mediaUrl` aqui é digitado/colado) ficam fora desta sprint.
...
2. Mutations (`Create` / `Update`) invalidam
   `quickRepliesControllerListQueryKey()` após sucesso.
3. Soft-delete via `PATCH active=false` dentro de `AlertDialog`,
   mantendo o padrão consagrado em Tags/Departments.
...
6. Filtros: `search` por shortcut/message (debounced via `useDeferredValue`)
   - Select Status (Ativos/Inativos, default Ativos) + Select Escopo
     (Todos/Globais/Pessoais, default Todos — "Todos" omite o param
     `scope`) + Switch "Apenas as minhas" (envia `mine=true` quando ON).
...
10. Asterisco vermelho via `<FieldLabel required>` apenas em `shortcut` e
    `message`. `scope` tem default `PERSONAL`, `active` tem default
    `true`, `mediaUrl`/`mediaMimeType` são opcionais — campos sem
    asterisco.
11. Backend recusa `scope` no `PATCH` (campo omitido do
    `UpdateQuickReplyDto`). Em modo edit, o Select de Escopo é desabilitado
    com helper "Para mudar o escopo, crie uma nova resposta e desative
    esta".
...
- `components/quick-replies/quick-replies-table-view.tsx` —
  ...
  | Mensagem (truncada com `title` no mouseover) | Escopo (Badge outline
  com `<BuildingIcon>` para Global / `<UserIcon>` para Pessoal) | Mídia
  (Badge outline com `<PaperclipIcon>` + mimetype, ou "—") | Status
  (Badge Ativo/Inativo) | Atualizado em (pt-BR `dateStyle:short
timeStyle:short`) | Ações (Editar | Desativar/Reativar contextual,
  ou "Apenas leitura" quando `canEditItem(item) === false`).
- `components/quick-replies/quick-replies-table.tsx` — fetcher +
  filtros (`InputGroup` no search com `SearchIcon`, Select Status,
  Select Escopo, `Switch` "Apenas as minhas"). Calcula `canEditItem` a
  partir de `useCurrentUser()`: AGENT só edita `scope === 'PERSONAL'`
  (PERSONAL alheio não aparece na lista — backend filtra), demais roles
  editam tudo. Monta dialogs internos (`QuickReplyDialog` em modo edit +
  `DeactivateQuickReplyDialog`) + handler inline de Reativar
  (`PATCH active=true` + toast + invalidate).
- `components/quick-replies/quick-reply-dialog.tsx` — RHF +
  ...
  restrição por role e por mode), active (Switch). Tratamento de erros:
  409 → `setError('shortcut')`; 400 com `errors[]` → `setError` por
  field (filtro pelas keys de `DEFAULT_VALUES`); 403 → `setError('root',
...
- `components/quick-replies/quick-reply-dialog-trigger.tsx` — botão
  ...
  Component permanece).
- `components/quick-replies/deactivate-quick-reply-dialog.tsx` —
  `AlertDialog` encapsulando
  `useQuickRepliesControllerUpdate({ data: { active: false } })` +
  invalidate.

### Testes (Vitest + RTL)

- `quick-replies-table-view.test.tsx` (17 casos) — 4 estados;
  emptyMessage customizada; atalho com prefixo "/"; badge Global vs
  Pessoal; mensagem truncada com `title`; mídia com mimetype;
  traço quando sem mídia; badge Inativo; Editar/Desativar/Reativar;
  ocultar Desativar em item inativo; ocultar ações + render "Apenas
  leitura" quando `canEditItem` retorna false.
- `quick-reply-dialog.test.tsx` (10 casos) — create com sucesso
  (payload contém `scope='PERSONAL'` por default); 409 inline em
  `shortcut`; validações `min(1)` de shortcut e mensagem; regex de
  shortcut rejeitando espaços; pareamento mediaUrl ↔ mediaMimeType
  (ambos os sentidos); edit pré-preenche e PATCH **não inclui** scope;
  scope desabilitado em edit com helper; AGENT em create vê scope fixo
  em PERSONAL com helper específico; 403 mostra erro de root com
  mensagem de admin.
- `deactivate-quick-reply-dialog.test.tsx` (3 casos) — confirmação +
  PATCH active=false; cancelar não chama o backend; toast de erro em
  falha.
...
- [x] `/configuracoes/quick-replies` renderiza header (título + CTA "Nova
      resposta rápida") + tabela tipada + empty state contextual pelo filtro
      Status.
- [x] AGENT acessa a rota e cria/edita/desativa/reativa apenas suas
      PERSONAL. COMPANY aparecem read-only com badge "Global" e sem botões.
- [x] SUPERVISOR/ADMIN cria/edita COMPANY livremente; vê suas PERSONAL
  - todas COMPANY (PERSONAL alheio é filtrado pelo backend).
...
- [x] `DeactivateQuickReplyDialog` desativa via PATCH active=false;
      pós-sucesso toast + invalidate.
- [x] Botão "Reativar" inline em items inativos chama PATCH active=true
      direto + toast + invalidate (sem AlertDialog).
- [x] Search com `<InputGroup>` + `<SearchIcon>` debounced + Select
      Status + Select Escopo + Switch "Apenas as minhas" alimentam a query.
...
- [x] Testes RTL: 17 + 10 + 3 = 30 novos casos para Quick Replies, mais
